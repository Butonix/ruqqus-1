...
def money_stats(v):
    ...
    now=int(time.time())
    intake=[x for x in g.db.query(PayPalTxn.usd_cents).filter(PayPalTxn.status==3, PayPalTxn.created_utc>midnight_year_start).all()]
    intake=sum([int(x - (x*0.029)- 30 ) for x in intake])
