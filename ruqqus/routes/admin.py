...
def participation_stats(v):
    ...
    data = {"valid_users": g.db.query(User).filter_by(is_deleted=False).filter(or_(User.is_banned == 0, and_(User.is_banned > 0, User.unban_utc > 0))).count(),
            ...
            "active_comments": g.db.query(Comment).join(Comment.post).filter(Submission.created_utc>cutoff).count(),
            "archived_comments": g.db.query(Comment).join(Comment.post).filter(Submission.created_utc<cutoff).count(),
