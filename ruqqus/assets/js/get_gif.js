      ...
      $.ajax({
        ...
        if (max < 15 && max > 0) {
          for (var i = 0; i <= max; i++) {
            ...
            container.innerHTML += ('<div class="card-columns p-3"><div class="card bg-secondary gif-keyboard-option" style="overflow: hidden" data-dismiss="modal" aria-label="Close" onclick="insertGIF(\'' + gifURL[i] + '\')"><img class="img-fluid" src="' + gifURL[i] + '"></div></div>');
          }
        }
        else if (max <= 0) {
