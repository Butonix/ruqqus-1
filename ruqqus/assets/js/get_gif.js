      ...
      $.ajax({
        ...
        if (max < 15 && max > 0) {
          for (var i = 0; i <= max; i++) {
            ...
            container.innerHTML += ('<div class="card bg-gray-200" style="overflow: hidden" data-dismiss="modal" aria-label="Close" onclick="insertGIF(\'' + gifURL[i] + '\',\'' + commentFormID + '\')"><div class="gif-cat-overlay"></div><img class="img-fluid" src="' + gifURL[i] + '"></div>');
            noGIFs.innerHTML = null;
          }
        }
        else if (max <= 0) {
