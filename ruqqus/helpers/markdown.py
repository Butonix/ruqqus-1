...
class CustomRenderer(HTMLRenderer):
    ...
    def render_user_mention(self, token):
        ...
        return f'{space}<a href="{user.permalink}"><span class="d-inline-block"><img src="/@{user.username}/pic/profile" class="profile-pic-20 mr-1">@{user.username}</span></a>'
    ...
    def render_board_mention(self, token):
        ...
        if not board or board.is_banned:
            ...
            return f'{space}<a href="{board.permalink}"><span class="d-inline-block"><img src="/+{board.name}/pic/profile" class="profile-pic-20 align-middle mr-1">+{board.name}</span></a>'
