...
def prep_database():
    ...
    RETURNS float AS '
      SELECT CAST(($1.ups - $1.downs) AS float)/((CAST(($1.age+1000) AS FLOAT)/6.0)^(1.0/2.0))
      '
    LANGUAGE SQL
    IMMUTABLE
    RETURNS NULL ON NULL INPUT;
    ...
    RETURNS float AS '
      SELECT SQRT(CAST(($1.ups * $1.downs) AS float))/((CAST(($1.age+1000) AS FLOAT)/6.0)^(1.0/2.0))
      '
    LANGUAGE SQL
    IMMUTABLE
    RETURNS NULL ON NULL INPUT;
    ...
    RETURNS float AS '
      SELECT CAST($1.comment_count AS float)/((CAST(($1.age+1000) AS FLOAT)/6.0)^(1.0/2.0))
    '
    LANGUAGE SQL
    IMMUTABLE
    RETURNS NULL ON NULL INPUT;
