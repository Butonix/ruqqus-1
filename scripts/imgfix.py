...
for post in db.query(Submission).options(lazyload('*'), joinedload(Submission.submission_aux)).filter_by(domain_ref=1, is_deleted=False).order_by(Submission.id.asc()).all():
    try:
